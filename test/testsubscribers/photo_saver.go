...
func (ps PhotoSaver) Handle(ev domain.Event) {
	...
	case domain.AtendeeRegisteredEvent:
		file, err := os.OpenFile(path.Join(ps.Dir, ev.Atendee.Photo.Name), os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o755)
		if err != nil {
			log.Print("could not open image for write: ", err)
		}
		defer file.Close()
		err = jpeg.Encode(file, ev.Atendee.Photo.Img, &jpeg.Options{
			Quality: 80,
		})
		if err != nil {
